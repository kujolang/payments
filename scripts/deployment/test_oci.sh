...
for target in gateway agent-probe harness-probe; do
    docker build --platform linux/amd64 -f deployment/Dockerfile --target "$target" -t "kujo-payments-$target:local" .
done
for test in domain bindings contracts lifecycle storage observations execution gateway; do
