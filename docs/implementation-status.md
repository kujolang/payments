...
The exact privileged seed also runs in normal host CI. Its local synthetic test passed using the available Kujo 1.3.1 release interpreter; the previously recorded local debug executable is no longer present. This local result is not Linux containment evidence. Both pinned Linux jobs passed at `6cb7512`, [run 34744602713](https://github.com/kujolang/payments/actions/runs/34744602713). Artifact 10314045490 matched its archive digest and all six manifest entries; all three new enrollment positive controls, both 30-file probes and complete cleanup were verified. Evidence: `docs/evidence/enrollment-containment-ci.json`; raw logs/archive: `.local/evidence/34744602713/`. No deployment, provider identity, authenticated operator renderer or live-account guarantee is added.


## Containment controller failure diagnostics

The synthetic containment controller now suppresses raw subprocess exceptions and captured stdout/stderr in readiness/probe/Workcell failure assertions. Python timeout and launch exceptions can otherwise include secret-bearing argument arrays; captured decoding errors can expose output. Three actual local subprocess fixtures exercise timeout with emitted private output, missing executable with a private path, and invalid UTF-8 after a private sentinel; formatted exceptions contain no sentinel. Workcell artifact scans now also check relative filenames before evidence export. This fixes the test-controller diagnostic boundary, not production telemetry or provider conformance. Both pinned Linux jobs passed at `f35c00f`, [run 34745022340](https://github.com/kujolang/payments/actions/runs/34745022340). Artifact 10314160911 matched its archive digest and all six manifest entries; all three new enrollment positive controls, both 30-file probes and complete cleanup were verified. Evidence: `docs/evidence/controller-diagnostics-ci.json`; raw logs/archive: `.local/evidence/34745022340/`.


## Repeated MCP runtime-cost measurements

The existing real HTTP/MCP conformance suite now records eight warm replay/status samples each, response-envelope bytes and actual native process counts through a fixture-only observer. It checks five repeated catalog discoveries and a 250 ms idle window, and enforces explicit descriptor/summary/envelope byte budgets. Local HTTP/SDK/MCP conformance passed on Kujo 1.3.1 and Node 24.20.0; no financial claims were created. See `docs/performance.md` and source/runtime-hashed `docs/evidence/mcp-performance-local.json`. The same complete HTTP/SDK/MCP conformance path also passed using the official checksum-verified Kujo 1.4.0 macOS x64 binary, stored only in `.local/runtime/kujo-1.4.0-macos-x64/`; the system installation is unchanged. Its separate baseline and release provenance are in `docs/evidence/mcp-performance-1.4-local.json`. These are instrumented local baselines, not token counts, supported-provider benchmarks or production latency claims. Both pinned Linux jobs passed at `68f4b18`, [run 34745435820](https://github.com/kujolang/payments/actions/runs/34745435820). Artifact 10314011794 matched its archive digest and all six manifest entries; all three new enrollment positive controls, both 30-file probes and complete cleanup were verified. Evidence: `docs/evidence/mcp-performance-ci.json`; raw logs/archive: `.local/evidence/34745435820/`. Broader release performance evaluation remains open.


## Consistent embedded principal admission

Tracing the compatibility gate found lifecycle paths that derived the three-field scope without first rejecting additional or malformed principal fields. Preparation/provider authorization, observation acceptance, internal reconciliation, execution fast paths, the worker and private review/delivery now apply the existing `ability_principal_compatible` check before store access. Canonical reconciliation already had this check. No digest algorithm, definition, receipt, schema or provider SPI changed. The HTTP gateway already rejected these profiles; this closes a trusted embedding consistency gap, not a demonstrated remote authentication bypass.

The regression fails before the fix on extra claims reaching preparation lookup. Eleven malformed/incompatible principals across ten entry points now return `unsupported_principal` without touching a real persistent store-access counter, with a compatible-principal positive control. The full uninterrupted local host suite passed with the checksum-verified Kujo 1.4.0 macOS x64 interpreter, including legacy identity vectors, execution/operator/provider flows, concurrency and fault tests, optional integrations, private enrollment and Fence. Log: `.local/application-identity-host.log`; initial failing admission regression: `.local/application-identity-before.log`. Both pinned Linux jobs passed at `3650888`, [run 34745825781](https://github.com/kujolang/payments/actions/runs/34745825781). Artifact 10314291932 matched its archive digest and all six manifest entries; all three new enrollment positive controls, both 30-file probes and complete cleanup were verified. Evidence: `docs/evidence/application-identity-ci.json`; raw logs/archive: `.local/evidence/34745825781/`. See `docs/identity-compatibility.md`. Broader versioned identity migration and release gates are not declared complete by this regression.


## Frozen Link issuance/submission journal admission

Recovery review found that the older private payment journal openers used `CREATE IF NOT EXISTS`, silently recreating missing constraints. They now reuse the existing exact-schema validator through a provider-private group initializer. The legacy authorization, submission and provider-preparation groups are validated in one transaction. New factory journals create all thirteen objects atomically; an existing partial factory profile cannot be silently completed. Missing/changed objects, foreign tables, submission-only profiles and future user versions fail closed. The supported standalone submission opener requires authorization first; identifiers, SQL constraints, rows, claims, user_version zero and the generic provider SPI are unchanged.

The targeted fixture passed legacy populated-row/claim preservation, 26 missing/changed objects, foreign/future profiles, submission-first denial and concurrent initialization. The initial full host run caught the omitted provider-preparation group on factory reopen; admission was corrected to include that frozen group, and the actual Link provider suite passed. The full uninterrupted host suite then passed for the final thirteen-object implementation using the verified Kujo 1.4.0 macOS x64 runtime, including 26 schema mutations, legacy data/claim preservation, concurrent initialization, the full provider lifecycle and Fence (36 files, 14 forbidden edges and three mutations). Final log: `.local/link-journal-final-host.log`; the earlier failed run is retained at `.local/link-journal-host.log`. Both pinned Linux jobs passed at `b63b66d`, [run 34746396831](https://github.com/kujolang/payments/actions/runs/34746396831). Artifact 10314710807 matched its archive digest and all six manifest entries; all six private positive controls, both 30-file probes and complete cleanup were verified. Evidence: `docs/evidence/link-journal-ci.json`; raw logs/archive: `.local/evidence/34746396831/`. See `docs/link-payment-journal.md`. This is structural admission, not proof of complete history or a coordinated backup/restore implementation.


## Published recovery snapshot verification

Added a bounded read-only maintenance verifier for the existing snapshot format. It validates publication hash/private file properties, required core schema and migration lineage, integrity/foreign keys, quarantine/paused evidence, claimed-execution normalization and absent sidecars, with post-read hash/identity checks. It reports extra application schema objects without claiming their semantics are verified. No source authentication, history completeness, live re-enable or provider-journal recovery claim is made.

The dedicated local fixture passed current/frozen-v1 and repeated exports, unchanged artifact bytes, 20 tamper/quarantine/path cases and bounds. Existing recovery snapshot regressions also passed on verified Kujo 1.4.0. Initial test construction used an audit row without a declared foreign key; it was corrected to an actual foreign-key violation in incidents. Both pinned Linux jobs passed at `b116c78`, [run 34747021106](https://github.com/kujolang/payments/actions/runs/34747021106). Artifact 10313829709 matched its archive digest and all six manifest entries; six private positive controls, both 30-file probes and complete cleanup were verified. Source-scoped evidence: `docs/evidence/recovery-verification-ci.json`; raw synthetic logs/archive: `.local/evidence/34747021106/`. See `docs/recovery-snapshots.md`.


## Frozen-profile compatibility decision

The application audit distinguishes canonical definition admission, approval binding and nested runtime request fingerprints. The frozen Payments profile does not require a general Ability journal migration: it admits full definitions with v2, restricts approval inputs/principals and independently binds payment/replay state. The legacy nested request fingerprint is explicitly not treated as collision-resistant. No hash, receipt, definition, runtime or financial claim was changed. See `docs/identity-compatibility.md` for each boundary, trusted-store assumptions and conditions requiring another review. The upstream general serializer issue remains open; this closes only the Payments-specific compatibility decision.

Ten targeted commands passed on verified Kujo 1.4.0 at source `b116c78`: independent definition vectors in both modes, approval binding, principal profile and embedded admission, actual execution/gateway, interrupted request/cancellation and reconciliation. Local log: `.local/compatibility-audit.log`; source-scoped record: `docs/evidence/compatibility-audit.json`. The same source passed the full hosted host/containment suite in run 34747021106. This is not a new full local host-suite run or a real-money acceptance claim.


## Declared private-journal recovery sets

`scripts/maintenance/recovery_set.py` adds privileged create/verify commands for a core journal plus up to seven declared private SQLite journals. It acquires all write reservations before capture, copies committed WAL through SQLite backup, reuses permanent core quarantine and publishes a bounded manifest only after every copy finishes. Verification checks fixed member paths, private owned files, aggregate byte limits, member hashes/integrity and core quarantine. Private journal schemas remain opaque; no credential installer, provider-specific restore semantics, live re-enable or financial-history merge was added.

The actual private seed fixture covers five journals, writer exclusion against every member, committed WAL, retained synthetic credentials only in private members, immutable verification, denied resume, 13 tamper cases, size/lock limits, copy failure and actual SIGKILL. The targeted fixture and full uninterrupted local host suite passed on verified Kujo 1.4.0 at `3725000`; `.local/recovery-set-host.log` and `docs/evidence/recovery-set-local.json` record the source/runtime scope. Both jobs in hosted run 34747825786 passed. Artifact 10315355298 matched its archive digest and all six manifest entries; six private positive controls, both 30-file probes and complete cleanup were verified. Source-scoped evidence: `docs/evidence/recovery-set-ci.json`; raw synthetic logs/archive: `.local/evidence/34747825786/`. The directory is credential-bearing and is not encrypted by this tool: backup infrastructure must provide custody, access control, encryption and retention. A common database barrier cannot rule out remote effects already in flight or detect an omitted journal. Copied credential generations must not be refreshed or installed into a normal executor. See `docs/recovery-snapshots.md`. The supported financial-history/operational restore gate remains open.
