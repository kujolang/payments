...
The dedicated local fixture passed current/frozen-v1 and repeated exports, unchanged artifact bytes, 20 tamper/quarantine/path cases and bounds. Existing recovery snapshot regressions also passed on verified Kujo 1.4.0. Initial test construction used an audit row without a declared foreign key; it was corrected to an actual foreign-key violation in incidents. Hosted verification of this maintenance addition is pending. See `docs/recovery-snapshots.md`.
