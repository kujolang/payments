...
The targeted fixture passed legacy populated-row/claim preservation, 26 missing/changed objects, foreign/future profiles, submission-first denial and concurrent initialization. The initial full host run caught the omitted provider-preparation group on factory reopen; admission was corrected to include that frozen group, and the actual Link provider suite passed. The full host suite is rerunning for the final thirteen-object implementation; hosted verification remains pending. See `docs/link-payment-journal.md`. This is structural admission, not proof of complete history or a coordinated backup/restore implementation.
