...
The regression fails before the fix on extra claims reaching preparation lookup. Eleven malformed/incompatible principals across ten entry points now return `unsupported_principal` without touching a real persistent store-access counter, with a compatible-principal positive control. The full uninterrupted local host suite passed with the checksum-verified Kujo 1.4.0 macOS x64 interpreter, including legacy identity vectors, execution/operator/provider flows, concurrency and fault tests, optional integrations, private enrollment and Fence. Log: `.local/application-identity-host.log`; initial failing admission regression: `.local/application-identity-before.log`. Hosted verification of this admission change is pending. See `docs/identity-compatibility.md`. Broader versioned identity migration and release gates are not declared complete by this regression.
