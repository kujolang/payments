...
The isolated runtime change was merged through [Kujo PR #9](https://github.com/kujolang/kujo/pull/9) as `e9e5a67762ada4e622dff5dcde357eafc1bb8e72`, excluding unrelated hardening work. All 22 reported PR checks passed for the reviewed head, including the full release gate and platform checks. Local validation also covered all eight routed HTTP integration tests, including duplicate-header preservation in both runtimes, and this repository's synthetic loopback endpoint test with the rebuilt binary. A merged source change is not a released dependency or provider-backed sandbox acceptance. Recheck release artifacts before changing the installation requirements.

A [manual development artifact run](https://github.com/kujolang/kujo/actions/runs/34878190347) builds that exact merged source without tag-based publication. Its Intel macOS archive passed local endpoint, private operator and crash-recovery fixtures after checksum, source-metadata and archive/npm executable equality checks. [Artifact evidence](evidence/runtime-development-artifacts.json) records exact hashes and tested scope. The embedded version is still `1.4.0`; the source commit and binary hash distinguish it from the released v1.4.0 executable. This evidence does not establish Link connectivity or a provider-backed payment, and Actions artifacts are temporary development downloads rather than a stable installer release.

That manual run completed successfully for all five platforms and neutral npm packaging; both publication jobs were skipped. All platform archives passed the integrity checks recorded above, but only the Intel macOS executable was exercised locally with Payments. Use the exact verified standalone executable for development testing. Do not install the neutral development npm tarball alone: its optional dependencies still name published `1.4.0` platform packages, which can resolve to the older released runtime. npm installation acceptance and a versioned public release remain separate gates.
