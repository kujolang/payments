...
The only HTTP route is `POST /v1/payments` with JSON, bearer authentication and `request` or `inspect`. Credential records bind a principal and allowed operations. The server accepts no caller principal, financial approval or execute endpoint. Response/error projections are bounded. The client rejects redirects, uses DNS pinning and requires HTTPS except explicitly enabled loopback fixtures. Unknown transport outcomes must be inspected with the original request identity.
