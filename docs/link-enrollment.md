...
A callback returning true is a trusted-host assertion, not independent proof of the provider account. Link's reviewed token/user-info material does not supply the documented stable payer subject needed to close that gate. Enrollment success must not be substituted for verified account identity or for a separate financial approval.
